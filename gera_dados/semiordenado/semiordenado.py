...
data_gen = pd.DataFrame(columns=['SCORE'])

# data_gen['VALUE'] = np.around(rng.random(int(data.size)) * rng.choice(int(data.max), size=int(data.size),
# replace=True), int(data.decimals)) data_gen['ID'] = [hash(str(x)) % (10 ** data.hashsize) for x in data_gen['VALUE']]
